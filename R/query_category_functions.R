...
dir.create('./wikidata_categories', showWarnings = FALSE)

#' Query Wikidata for category using the administrative entity (1)
#'
#' This functions query for Wikidata items' category ('Instance of' statement, P31) using the administrative entity. You have to look first for the wikidata location identifier on wikidata.org ('Q_____')
...
#' @export
query_location_category_1 <- function(city_code){
  ...
  download.file(paste0(api_url, query, "&format=json"), paste0('/wikidata_categories/', city_code, '.txt'))
}

#' Query Wikidata for category using a radius around the city (2)
#'
#' This functions query for Wikidata items's category using a radius around the city. You have to look first for the wikidata location identifier on wikidata.org ('Q_____') and to set a radius in km.
#' @param city_code The Wikidata identifier ('Q____')
...
#' @export
query_location_category_2 <- function(city_code, radius){
...
#' @export
query_location_category_3 <- function(city_code, first_corner_city_code, first_city_corner, second_corner_city_code, second_city_corner){
...
#' Read the list of cateories related to items
#'
#' This function reads the list of categories that has been downloaded
#' @param city_code The Wikidata identifier ('Q____')
#' @return A dataframe with the Wikidata identifier of the item and its category (there could be multiple categories for each item)
#' @export
read_category_list <- function(city_code){
  json <- jsonlite::fromJSON(paste0('/wikidata_categories/', city_code, '.txt'), simplifyDataFrame = TRUE)
